...
class DynamicFirewall(app_manager.RyuApp):
    ...
    def handler_datapath(self, ev):
        ...
        if ev.dp.id == 1:
            self.fwc.set_enable("", "0000000000000001")

            rules = ['{"nw_src": "10.0.0.1/32", "nw_dst": "10.0.0.2/32", "nw_proto": "ICMP"}',
            ...
            for rule in rules:
                ...
                self.fwc.set_rule(res, "0000000000000001")
...
class FirewallController():
    ...
    def set_log_enable(self, dummy, switchid, **_kwargs):
        return self._access_module(switchid, 'set_log_enable', waiters=self.waiters)

    # PUT /firewall/log/disable/{switchid}
    def set_log_disable(self, dummy, switchid, **_kwargs):
        return self._access_module(switchid, 'set_log_disable', waiters=self.waiters)
...
class Firewall(object):
    ...
    def _set_log_status(self, is_enable, waiters):
        ...
        if waiters:
            ...
            # Initialize.
            flow = self._to_of_flow(cookie=0, priority=LOG_FLOW_PRIORITY, match={}, actions=actions)
            self.ofctl.mod_flow_entry(self.dp, flow, cmd)
